...
class OrgRemoveMemberAuditEntryReason(sgqlc.types.Enum):
    __schema__ = github_schema
    __choices__ = ('TWO_FACTOR_REQUIREMENT_NON_COMPLIANCE', 'SAML_EXTERNAL_IDENTITY_MISSING', 'SAML_SSO_ENFORCEMENT_REQUIRES_EXTERNAL_IDENTITY', 'USER_ACCOUNT_DELETED')
...
class AddPullRequestReviewThreadInput(sgqlc.types.Input):
    __schema__ = github_schema
    __field_names__ = ('path', 'body', 'pull_request_review_id', 'line', 'side', 'start_line', 'start_side', 'client_mutation_id')
...
class TreeEntry(sgqlc.types.Type):
    __schema__ = github_schema
    __field_names__ = ('mode', 'name', 'object', 'oid', 'repository', 'submodule', 'type')
