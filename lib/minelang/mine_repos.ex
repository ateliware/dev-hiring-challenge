defmodule Minelang.MineRepos do
  use GenServer

  require Ecto.Query
  alias Minelang.{MineServer, Repo, Repository}
  ...
  def init(orgs) do
    orgs =
      Ecto.Query.from(r in Repository, select: r.org)
      |> Repo.all()
      |> Enum.concat(orgs)
  ...
  def handle_info(:start_miner, orgs) do
    for org <- orgs do
      case Repo.get_by(Repository, org: org) do
        nil ->
